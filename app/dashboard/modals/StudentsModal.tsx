...
export default function StudentsModal({ schoolId, onClose }: { schoolId: string; onClose: () => void }) {
  ...
  function fillEdit(s: Student) {
    setEditStudent(s)
    setNisn(s.nisn); setNama(s.nama); setKelas(s.kelas)
    setStatus(s.status); setPesan(s.pesan)
    setTab('tambah')
  }
  ...
  return (
    <Modal title={editStudent ? `✏️ Edit: ${editStudent.nama}` : '👨‍🎓 Data Siswa'} onClose={onClose}>
      {/* Tabs */}
      <div className="flex gap-2 mb-4">
        {(['tambah', 'daftar'] as const).map(t => (
          <button key={t} onClick={() => { setTab(t); if(t==='tambah') resetForm() }}
            className={`flex-1 py-2 rounded-xl text-sm font-bold transition-colors ${
              tab === t ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
            }`}>
            {t === 'tambah' ? (editStudent ? '✏️ Edit Siswa' : '➕ Tambah') : `📋 Daftar (${students.length})`}
          </button>
        ))}
      </div>
      ...
      {tab === 'tambah' && (
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <input value={pesan} onChange={e => setPesan(e.target.value)} placeholder="Tulis pesan sendiri atau pilih di bawah..."
                ...
                <div key={s.id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-200">
                  ...
                  <div className="flex gap-1 ml-2">
                    <button onClick={() => fillEdit(s)}
                      className="text-blue-400 hover:text-blue-600 p-1.5 rounded-lg hover:bg-blue-50 transition-colors text-sm">
                      ✏️
                    </button>
                    <button onClick={() => deleteStudent(s.id)}
                      className="text-red-400 hover:text-red-600 p-1.5 rounded-lg hover:bg-red-50 transition-colors text-sm">
                      🗑️
                    </button>
                  </div>
                </div>
