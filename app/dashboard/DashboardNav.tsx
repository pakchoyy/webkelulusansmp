...
export default function DashboardNav({ namaSekolah, slug }: { namaSekolah: string; slug: string }) {
  ...
  const links = [
    ...
    { href: '/dashboard/upload', label: '📥 Upload Excel' },
    ...
    <header className="bg-white border-b-2 border-black sticky top-0 z-30">
      <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-black text-gray-900 text-sm truncate">{namaSekolah}</span>
        </div>
        <nav className="flex items-center gap-1 overflow-x-auto">
          ...
          {slug && (
            <a
              href={`/${slug}`}
              target="_blank"
              className="whitespace-nowrap px-3 py-1.5 rounded-lg text-xs font-bold text-green-700 hover:bg-green-50 transition-colors"
            >
              🔗 Lihat Halaman
            </a>
          )}
